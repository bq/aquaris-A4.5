...
//LC--zbl--20150609 add for runin BAT_OVER_SOC_RECHARGE start
...
#endif	
//LC--zbl--20150609 add for runin BAT_OVER_SOC_RECHARGE end
...
#if 0 //def BAT_LOW_TEMP_PROTECT_ENABLE //LC---zbl---rm---20150415---for DEND-642
...
void mt_battery_GetBatteryData(void)
{
	...
	if( upmu_is_chr_det() == KAL_TRUE ) {
		//ICharging = battery_meter_get_charging_current();//zbl rm 
                ICharging =  get_bat_charging_current_level(); //zbl add
                ICharging =  ICharging/100;
	        charger_vol = battery_meter_get_charger_voltage();//LC--zbl--modify-20150511
	} else {
		ICharging = 0;
                charger_vol = 0;//LC--zbl--modify-20150511
	}
...
#endif
		#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
		if (BMT_status.charger_vol >= 5500) { //V_CHARGER_MAX //LC--zbl--modify--20150501
		#else
		if (BMT_status.charger_vol >= 5500) { //v_charger_max //LC--zbl--modify--20150501
		#endif
...
/*
	...
	else
	{
        ...
        g_BatteryNotifyCode |= 0x0040;
	#endif
        battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) in range(normal)\n", BMT_status.temperature);		
    }
*/
...
static void mt_battery_thermal_check(void)
{
	...
	if ((g_battery_thermal_throttling_flag == 1) || (g_battery_thermal_throttling_flag == 3)) {
		...
		/* ignore default rule */
#else
	    if (BMT_status.temperature >= 58) { //60 zbl modify 20150305
#if defined(CONFIG_POWER_EXT)
			battery_log(BAT_LOG_CRTI,
		   ...
		   || (g_platform_boot_mode == ATE_FACTORY_BOOT)) {
			...
			struct power_supply *bat_psy = &bat_data->psy;
					...
					battery_log(BAT_LOG_CRTI,
			"[Battery] Tbat(%d)>=58, system need power down.\n",
			BMT_status.temperature);
...
void do_chrdet_int_task(void)
{
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if (BMT_status.bat_vol > 0) {
			BMT_status.charger_vol = battery_meter_get_charger_voltage();//LC--zbl--modify-20150511
			if(BMT_status.charger_type == STANDARD_CHARGER)
				BMT_status.ICharging = AC_CHARGER_CURRENT/100;//LC--zbl--modify-20150511
			else if(BMT_status.charger_type == STANDARD_HOST)
				BMT_status.ICharging = USB_CHARGER_CURRENT/100;//LC--zbl--modify-20150511
