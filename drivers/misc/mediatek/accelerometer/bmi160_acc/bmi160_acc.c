...
//#include <linux/hwmsensor.h>		//swf delete
//#include <linux/hwmsen_dev.h>		//swf delete
//#include <linux/sensors_io.h>		//swf delete
#include <accel.h>

//include <mach/mt_typedefs.h>  		//swf delete
//#include <mach/mt_gpio.h>		//swf delete
//#include <mach/mt_pm_ldo.h>		//swf delete
...
//#include <linux/hwmsen_helper.h>  //swf delete


/*************************LC 20151031 swf add start*******************/
#include <hwmsensor.h>	//swf add for struct hwm_sensor_data* gsensor_data;
#include <sensors_io.h>  //swf add for struct SENSOR_DATA sensor_data;
...
static struct GSENSOR_VECTOR3D gsensor_gain;  //swf modify
...
static ssize_t store_cpsopmode_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	//error = strict_strtoul(buf, 10, &data); //swf delete for api change
		error = kstrtoul(buf, 10, &data);  //swf add for api change
	if (error)
	{
		return error;
	}
...
static ssize_t store_cpsrange_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	//error = strict_strtoul(buf, 10, &data); //swf delete
	error = kstrtoul(buf, 10, &data);        //   swf
	if (error)
	{
		return error;
	}
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	struct hwm_sensor_data* gsensor_data;  //hwm_sensor_data* gsensor_data;   //swf modify
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			if((buff_out == NULL) || (size_out< sizeof(struct hwm_sensor_data))) //swf modify
			{
			...
			{
				gsensor_data = (struct hwm_sensor_data *)buff_out; //swf modify
		...
		void* buff_out, int size_out, int* actualout)
{
	int err = 0;
	int value;
	struct hwm_sensor_data* vrv_data;//swf modify
#if DEBUG
	struct i2c_client *client = bmi160_acc_i2c_client;
	struct bmi160_acc_i2c_data *data = obj_i2c_data;
#endif
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			{
				vrv_data = (struct hwm_sensor_data *)buff_out;//swf modify
				mutex_lock(&sensor_data_mutex);
		...
		void* buff_out, int size_out, int* actualout)
{
	int err = 0;
	int value;
	struct hwm_sensor_data* vla_value;//swf modify
#if DEBUG
	struct i2c_client *client = bmi160_acc_i2c_client;
	struct bmi160_acc_i2c_data *data = obj_i2c_data;
#endif
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			{
				vla_value = (struct hwm_sensor_data *)buff_out;//swf modify 
				mutex_lock(&sensor_data_mutex);
		...
		void* buff_out, int size_out, int* actualout)
{
	int err = 0;
	int value;
	struct hwm_sensor_data* vg_value;  //swf modify
#if DEBUG
	struct i2c_client *client = bmi160_acc_i2c_client;
	struct bmi160_acc_i2c_data *data = obj_i2c_data;
#endif
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			{
				vg_value = (struct hwm_sensor_data *)buff_out; //swf modify
				mutex_lock(&sensor_data_mutex);
...
static long bmi160_acc_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	char strbuf[BMI160_BUFSIZE];
	void __user *data;
	struct SENSOR_DATA sensor_data;//SENSOR_DATA sensor_data;  //swf modify
	long err = 0;
	int cali[3];
	//tad3sgh add ++
	...
	switch(cmd)
	{
		...
		case GSENSOR_IOCTL_READ_GAIN:
			...
			if(copy_to_user(data, &gsensor_gain, sizeof(struct GSENSOR_VECTOR3D))) //swf modify
			{
				err = -EFAULT;
				break;
			}
...
/*----------------------------------------------------------------------------*/
static int bmi160_acc_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)//swfprobe
{
	...
	obj->hw = hw;	//obj->hw = get_cust_acc_hw(); //swf modify
...
#ifdef   LC_DEVINFO_GSENSOR
	gsensor_devinfo_init(); //swf add for devinfo
#endif
...
static int  bmi160_acc_local_init(void)
{
	//struct acc_hw *hw = get_cust_acc_hw();  //swf delete
...
static int __init bmi160_acc_init(void)
{
	...
	const char *name = "mediatek,bmi160_acc";   //swf add 
	hw = get_accel_dts_func(name, hw);	//swf add

	GSE_FUN();
	//i2c_register_board_info(hw->i2c_num, &bmi160_acc_i2c_info, 1);  //swf delete
