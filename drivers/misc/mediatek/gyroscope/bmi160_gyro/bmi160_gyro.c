...
#define POWER_NONE_MACRO -1//MT65XX_POWER_NONE   //swf modify   mt_pm_ldo.h

#include <cust_gyro.h>  

//#include <hwmsensor.h>   //swf add 
#include <mt_boot_common.h> //swf add 
		...
		void *buff_out, int size_out, int *actualout)
{
	...
	struct hwm_sensor_data *gyroscope_data;  //hwm_sensor_data *gyroscope_data;  swf modify
...
#if 1   //swf modify  //after bmi160_acc make kernel ok  ,this should be opened
...
static int bmg_i2c_probe(struct i2c_client *client,const struct i2c_device_id *id)   //swfpro
{
	struct bmg_i2c_data *obj;
	...
	obj->hw = hw;//obj->hw = get_cust_gyro_hw();   //swf modify
...
{
    //struct gyro_hw *hw = get_cust_gyro_hw(); //swf delete 
    GYRO_FUN();    
    bmg_power(hw, 0);    
    i2c_del_driver(&bmg_i2c_driver);
    return 0;
...
static int __init bmg_init(void)
{
	//struct gyro_hw *hw = get_cust_gyro_hw();//swf delete
        ...
        const char *name = "mediatek,bmi160_gyro";  /*swf add*/
 	hw = get_gyro_dts_func(name, hw);

	GYRO_LOG("%s: bosch gyroscope driver version: %s\n",
	__func__, BMG_DRIVER_VERSION);

     //	i2c_register_board_info(hw->i2c_num, &bmg_i2c_info, 1); //swf delete
