...
#define AKM_AXIS_Z 2 
    ...
    /*combination*/ 
    databuf[AKM_AXIS_X] = (s16)(((u16)sensordata[1])&0x00ff | (((u16)(sensordata[2])) << 8)&0xff00);
    databuf[AKM_AXIS_Y] = (s16)(((u16)sensordata[3])&0x00ff | (((u16)(sensordata[4])) << 8)&0xff00); 
    databuf[AKM_AXIS_Z] = (s16)(((u16)sensordata[5])&0x00ff | (((u16)(sensordata[6])) << 8)&0xff00); 
    /*axis remap*/ 
    mag[obj->cvt.map[AKM_AXIS_X]] = obj->cvt.sign[AKM_AXIS_X]*databuf[AKM_AXIS_X]; 
    mag[obj->cvt.map[AKM_AXIS_Y]] = obj->cvt.sign[AKM_AXIS_Y]*databuf[AKM_AXIS_Y]; 
    mag[obj->cvt.map[AKM_AXIS_Z]] = obj->cvt.sign[AKM_AXIS_Z]*databuf[AKM_AXIS_Z]; 

    AKMDBG("mag raw data[%d, %d, %d][%d, %d, %d]\n", 
		    databuf[AKM_AXIS_X], databuf[AKM_AXIS_Y], databuf[AKM_AXIS_Z], mag[AKM_AXIS_X], mag[AKM_AXIS_Y], mag[AKM_AXIS_Z]); 
    AKMDBG("mag raw data map[%d, %d, %d] sign[%d, %d, %d]\n", 
		    obj->cvt.map[0], obj->cvt.map[1], obj->cvt.map[2], obj->cvt.sign[0], obj->cvt.sign[1], obj->cvt.sign[2]);
...
static long akm09911_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	AKMDBG(KERN_ERR"akm09911 cmd:0x%x\n", cmd);	
	switch (cmd)
	{
...
static long akm09911_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	AKMDBG("akm09911_compat_ioctl arg: 0x%lx, arg32: 0x%p\n",arg, arg32);
