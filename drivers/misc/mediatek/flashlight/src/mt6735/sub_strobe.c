...
static struct pinctrl_state  *frontflash_output0;
static struct pinctrl_state  *frontflash_output1;
...
int sub_FL_Init(void)
{
	...
	pinctrl_select_state(pinctrl, frontflash_en_output0);
	pinctrl_select_state(pinctrl, frontflash_output0);

	//if(mt_set_gpio_mode(GPIO_ENF,GPIO_MODE_00)){PK_DBG("[sub_flashlight] set gpio mode failed!! \n");}
    //if(mt_set_gpio_dir(GPIO_ENF,GPIO_DIR_OUT)){PK_DBG("[sub_flashlight] set gpio dir failed!! \n");}
    //if(mt_set_gpio_out(GPIO_ENF,GPIO_OUT_ZERO)){PK_DBG("[sub_flashlight] set gpio failed!! \n");}
    /*Init. to disable*/
    //if(mt_set_gpio_mode(GPIO_ENT,GPIO_MODE_00)){PK_DBG("[sub_flashlight] set gpio mode failed!! \n");}
    //if(mt_set_gpio_dir(GPIO_ENT,GPIO_DIR_OUT)){PK_DBG("[sub_flashlight] set gpio dir failed!! \n");}
    //if(mt_set_gpio_out(GPIO_ENT,GPIO_OUT_ZERO)){PK_DBG("[sub_flashlight] set gpio failed!! \n");}
...
void sub_FL_Flash_Mode(void)
{
	...
	pinctrl_select_state(pinctrl, frontflash_output1);
	pinctrl_select_state(pinctrl, frontflash_en_output1);

    //if(mt_set_gpio_mode(GPIO_ENT,GPIO_MODE_00)){PK_DBG("[sub_flashlight] set gpio mode failed!! \n");}
    //if(mt_set_gpio_dir(GPIO_ENT,GPIO_DIR_OUT)){PK_DBG("[sub_flashlight] set gpio dir failed!! \n");}
    //if(mt_set_gpio_out(GPIO_ENT,GPIO_OUT_ONE)){PK_DBG("[sub_flashlight] set gpio failed!! \n");}
//yzy add 
   // if(mt_set_gpio_mode(GPIO_ENF,GPIO_MODE_00)){PK_DBG("[sub_flashlight] set gpio mode failed!! \n");}
   // if(mt_set_gpio_dir(GPIO_ENF,GPIO_DIR_OUT)){PK_DBG("[sub_flashlight] set gpio dir failed!! \n");}
   // if(mt_set_gpio_out(GPIO_ENF,GPIO_OUT_ONE)){PK_DBG("[sub_flashlight] set gpio failed!! \n");}
...
static int frontflash_probe(struct platform_device *pdev)
{
	...
	frontflash_output0 = pinctrl_lookup_state(pinctrl, "front_flash_output0");
    if (IS_ERR(frontflash_output0))
	{
		err = PTR_ERR(frontflash_output0);
		dev_err(&pdev->dev, "fwq Cannot find pinctrl front_flash_output0!\n");
   	}
	...
	frontflash_output1 = pinctrl_lookup_state(pinctrl, "front_flash_output1");
    if (IS_ERR(frontflash_output1))
	{
		err = PTR_ERR(frontflash_output1);
		dev_err(&pdev->dev, "fwq Cannot find pinctrl front_flash_output1!\n");
   	}
...
static struct of_device_id frontflash_of_match[] = {
        { .compatible = "mediatek,mt6735-frontflash", },
        {},
};
