...
{	
	...
	for(i=0;i<num_of_emi_records;i++) {
		...
		if((emi_settings[i].ID[0]==(host->mmc->card->raw_cid[0]&0xFF000000)>>24) && (emi_settings[i].ID[1]==(host->mmc->card->raw_cid[0]&0x00FF0000)>>16) && (emi_settings[i].ID[2]==(host->mmc->card->raw_cid[0]&0x0000FF00)>>8) && (emi_settings[i].ID[3]==(host->mmc->card->raw_cid[0]&0x000000FF)>>0)
				// add the code to enhance ability of detecting more devices,for same seriers products
...
static struct timer_list emmc_timer;
static struct work_struct emmc_work;
static void emmc_work_func(struct work_struct *work)
{
        rom_size = msdc_get_user_capacity(mtk_msdc_host[0])/1024/1024;
	devinfo_register_emcp(mtk_msdc_host[0]);
}
static void emmc_timer_func(unsigned long data)
{
	if(mtk_msdc_host[0] == NULL      || 
           mtk_msdc_host[0]->mmc == NULL || 
	   mtk_msdc_host[0]->mmc->card == NULL) {
		mod_timer(&emmc_timer, jiffies + HZ);
	} else {
		schedule_work(&emmc_work);
	}
}
...
static void devinfo_flash_localinit(void)
{
	...
	printk("[DEVINFO_EMCP]start to %s\n", __func__);
  	...
  	setup_timer(&emmc_timer, emmc_timer_func, "success!");	
	INIT_WORK(&emmc_work, emmc_work_func);	
	mod_timer(&emmc_timer, jiffies + HZ);
}
