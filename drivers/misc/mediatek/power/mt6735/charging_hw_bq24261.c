...
//extern int is_mt6311_sw_ready(void);//zbl rm
...
static kal_uint32 is_chr_det(void)
{
    ...
    //val = mt6325_upmu_get_rgs_chrdet();//zbl rm
    val = pmic_get_register_value(PMIC_RGS_CHRDET);//zbl add
...
static kal_uint32 charging_hw_init(void *data)
{
    ...
    //in bq24261_set_ichg
    bq24261_config_interface(bq24261_CON4, 0x7, 0x1F,3); // ICHG to BAT zbl modify for enable 1.2A charging

    //zbl modify the termination current 100mA 20150326 
...
static kal_uint32 charging_set_cv_voltage(void *data)
{
    ...
    #else
    //PCB workaround
    //if(mt6325_upmu_get_swcid() == PMIC6325_E1_CID_CODE)//zbl rm
    if(pmic_get_register_value(PMIC_SWCID) == PMIC6325_E1_CID_CODE) //zbl add
    {
...
static kal_uint32 charging_set_hv_threshold(void *data)
{
    ...
    //mt6325_upmu_set_rg_vcdt_hv_vth(register_value);//zbl rm
    pmic_set_register_value(PMIC_RG_VCDT_HV_VTH,register_value);//zbl add
...
#else
    //*(kal_bool*)(data) = mt6325_upmu_get_rgs_vcdt_hv_det();//zbl rm
    *(kal_bool*)(data) = pmic_get_register_value(PMIC_RGS_VCDT_HV_DET);//zbl add
#endif
...
#else
    //pmic_read_interface( MT6325_CHR_CON7, &val, MT6325_PMIC_BATON_TDET_EN_MASK, MT6325_PMIC_BATON_TDET_EN_SHIFT);//zbl rm
    val = pmic_get_register_value(PMIC_BATON_TDET_EN);//zbl add
    battery_xlog_printk(BAT_LOG_FULL,"[charging_get_battery_status] BATON_TDET_EN = %d\n", val);
    if (val) {
    	//mt6325_upmu_set_baton_tdet_en(1);
    	//mt6325_upmu_set_rg_baton_en(1);//zbl rm
        pmic_set_register_value(PMIC_BATON_TDET_EN,1);
        pmic_set_register_value(PMIC_RG_BATON_EN,1);//zbl add
    	//*(kal_bool*)(data) = mt6325_upmu_get_rgs_baton_undet();//zbl rm
        *(kal_bool*)(data) = pmic_get_register_value(PMIC_RGS_BATON_UNDET);//zbl add
    } else {
...
static kal_uint32 charging_set_ta_current_pattern(void *data)
{
    ...
    BATTERY_VOLTAGE_ENUM cv_voltage = BATTERY_VOLT_04_400000_V;//BATTERY_VOLT_04_340000_V;//zbl modify
    #else
