...
//LC zgy 20151102 add for lcm info start
#ifdef CONFIG_LCT_DEVINFO_LCM
#include <linux/proc_fs.h>
#include <linux/seq_file.h>

BOOL mtkfb_find_lcm_driver_info(void)
{
	BOOL ret = FALSE;
	char *p, *q;
...
static int lcminfo_proc_show(struct seq_file *m, void *unused)
{
    printk("Enter lcm_proc_read.\n");
	if(mtkfb_find_lcm_driver_info())
	{
		printk(" LCM info:%s\n",mtkfb_lcm_name);
		seq_printf(m, "LCM:%s\n",mtkfb_lcm_name);
	}
		...
		seq_printf(m, "NULL\n");
	printk("Leave lcm_proc_read.\n");
	return 0;
}
static int lcminfo_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, lcminfo_proc_show, NULL);
}
static const struct file_operations lcminfo_proc_fops = {
    .open       = lcminfo_proc_open,
    .read       = seq_read,
    .llseek     = seq_lseek,
    .release	= single_release,
};
#endif
//LC zgy 20151102 add for lcm info end
...
////LC zgy 20151102 add for lcm info start
#ifdef CONFIG_LCT_DEVINFO_LCM
	proc_create("lcm_info", 0444, NULL, &lcminfo_proc_fops);
#endif
////LC zgy 20151102 add for lcm info end
	MSG_FUNC_LEAVE();
	return 0;
