...
//LC zgy 20151102 add for lcm devinfo start
#ifdef CONFIG_LCT_DEVINFO_LCM
#define CONFIG_LCT_DEVINFO_LCM_DEBUG

#include  <linux/dev_info.h>
struct devinfo_struct s_DEVINFO_lcm[10];   //suppose 10 max lcm device 
void devinfo_lcm_reg()
{
    int i=0;
    for(i = 0;i < lcm_count;i++)
    {
#ifdef CONFIG_LCT_DEVINFO_LCM_DEBUG
...
void DISP_DEVINFO_LCM_get(const char* lcm_name)
{
    ...
    for(i = 0;i < lcm_count;i++)
    {
        ...
        s_DEVINFO_lcm[i].device_version=slt_lcm_params->version;
...
#endif
//LC zgy 20151102 add for lcm devinfo end
...
//lc zgy 20151105 add to support gpio control start
...
//lc zgy 20151105 add to support gpio control end


//LC zgy 20151106 add lcm dev_info 
...
disp_lcm_handle *disp_lcm_probe(char *plcm_name, LCM_INTERFACE_ID lcm_id)
{
	...
	if (isLCMFound == false) {
        ...
        //LC zgy 20151102 add for lcm devinfo
#ifdef CONFIG_LCT_DEVINFO_LCM
	DISP_DEVINFO_LCM_get(plcm_name);
#endif
       //LC zgy 20151102 add for lcm devinfo

 //LC zgy 20151106 add for lcm devinfo
#ifdef SLT_DEVINFO_LCM
        lcm_get_readID_pin();
	lcm_devinfo_init();
